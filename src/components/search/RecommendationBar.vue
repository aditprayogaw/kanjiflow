...
<template>
    <div
        ...
        <div class="flex pt-3 gap-4 overflow-x-auto pb-4 custom-scrollbar min-h-35">
            <template v-if="kanjiStore.recommendations.length > 0">
                <div v-for="kanji in kanjiStore.recommendations" :key="kanji.char" @click="handleSelect(kanji)"
                    ...
                    class="group relative w-28 h-32 shrink-0 rounded-3xl border-2 flex flex-col items-center justify-center gap-1 cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div
                        class="absolute -top-2 -right-2 bg-[#4338CA] text-white text-[9px] font-black px-2 py-1 rounded-lg shadow-lg">
                        {{ kanji.match }}%
                    </div>

                    <span
                        class="text-4xl font-black text-slate-800 group-hover:scale-110 transition-transform duration-300">
                        {{ kanji.char }}
                    </span>

                    <div class="flex flex-col items-center mt-1">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{{ kanji.meaning
                            }}</span>
                        <div class="flex items-center gap-1 mt-1">
                            <div :class="kanji.match > 80 ? 'bg-emerald-400' : 'bg-amber-400'"
                                class="w-1 h-1 rounded-full"></div>
                            <span class="text-[9px] font-bold text-slate-400 italic">Level {{ kanji.level }}</span>
                        </div>
                    </div>
                </div>
            </template>
            ...
            <div v-else
                class="w-full border-2 border-dashed border-slate-100 rounded-3xl flex flex-col items-center justify-center py-8 bg-slate-50/50">
                <div
                    class="text-slate-300 italic text-[10px] font-black uppercase tracking-[0.2em] text-center px-4 leading-relaxed">
